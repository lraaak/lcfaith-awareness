...
const TheologySection: React.FC = () => {
    ...
    return (
        <section id="faith" className="py-32 px-6 bg-stone-50 relative border-y border-stone-200/40">
            <div className="max-w-7xl mx-auto space-y-24">
                <div ref={headerRef} className="flex flex-col md:flex-row justify-between items-end gap-8">
                    <div className="max-w-2xl space-y-4">
                        <span className="font-label text-primary font-bold tracking-[0.3em] uppercase text-xs block">Theological Reflection</span>
                        <h2 className="font-headline text-4xl md:text-7xl font-black text-stone-900 leading-none">The Theology of Growth.</h2>
                    </div>
                    <p className="text-stone-500 text-lg md:text-xl max-w-sm font-medium leading-relaxed italic">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
                    {verses.map((v, i) => (
                        <div key={i} ref={(el) => { if (el) versesRef.current[i] = el; }} className="bg-white p-12 rounded-3xl shadow-xl shadow-stone-200/50 space-y-8 border border-stone-100 flex flex-col justify-between group hover:-translate-y-2 transition-transform duration-500">
                            <div className="space-y-6">
                                <div className="w-10 h-10 bg-primary/5 rounded-full flex items-center justify-center">
                                    <span className="material-symbols-outlined text-primary text-xl">auto_stories</span>
                                </div>
                                <h3 className="font-headline text-2xl font-black text-stone-900 group-hover:text-primary transition-colors">{v.verse}</h3>
                                <p className="text-stone-800 text-xl font-medium leading-relaxed font-serif">
                                    {v.content}
                                </p>
                            </div>
                            <p className="text-stone-500 text-sm leading-relaxed border-t border-stone-100 pt-6">
                                {v.meaning}
                            </p>
                        </div>
                    ))}
                ...
                <div ref={bottomRef} className="grid grid-cols-1 md:grid-cols-2 gap-12 pt-12">
                    <div className="bg-stone-900 p-12 rounded-3xl text-white space-y-6 relative overflow-hidden group">
                        <h3 className="font-headline text-3xl font-black relative z-10">Human Dignity</h3>
                        <p className="text-stone-400 leading-relaxed text-lg relative z-10 font-medium">
                        ...
                        <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-primary/20 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-1000"></div>
                    </div>
                    <div className="bg-stone-100 p-12 rounded-3xl text-stone-900 space-y-6 relative overflow-hidden group border border-stone-200">
                        <h3 className="font-headline text-3xl font-black relative z-10">Preference for the Poor</h3>
                        <p className="text-stone-500 leading-relaxed text-lg relative z-10 font-medium">
                        ...
                        <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-zinc-200 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-1000"></div>
                    </div>
                </div>
            </div>
        </section>
